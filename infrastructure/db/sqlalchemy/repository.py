...
class SQLAlchemyRepository(BaseRepository[Model]):
    ...
    async def get_by_id(self, model: type[Model], id_: int) -> Model:
        session = self._session_manager.session()
    ...
    async def update_obj(self, model: type[Model], id_: int, **kwargs: Any) -> None:
        session = self._session_manager.session()
        query = update(model).where(model.id == id_).values(kwargs)  # type: ignore
        await session.execute(query)

    async def delete_obj(self, model: type[Model], id_: int) -> None:
        session = self._session_manager.session()
